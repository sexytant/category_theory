...
\chapter{準備}
\section{集合}
{\bf 集合}とは, ひとまず素朴に「ものの集まり」と定義される. 集合を構成する「もの」を{\bf 元}という.
\begin{Notation}
...
\end{Notation}
\begin{Def}
集合$A$の元のすべてが集合$B$の元であるとき{\bf $A$は$B$の部分集合である}という.  
\end{Def}
\begin{Notation}
...
\end{Def}
\begin{Notation}
$f$が集合$A$から集合$B$への写像であることを$f:A\rightarrow B$で表す.
\end{Notation}
\begin{caution}
以下では, 「{\bf 関数\index{かんすう@関数}}」と「写像」を同じ意味で用いる.
\end{caution}
\begin{Notation}
$f$が集合$A$から集合$B$への写像に関して, $b\in B$が$a\in A$に対応することを
\[
b=f(a)
\]
で表す.
...
f(a)=b
\]
となるとき{\bf $f$は全射である}という.
\end{Def}
\begin{Def}
集合$A$から集合$B$への写像$f$が, 単射であり, かつ全射であるとき{\bf $f$は全単射である}という.
\end{Def}
\begin{comment}
\begin{Def}
$R$が集合$A,B$の二項関係であるとする.
...
\subsection{合成と結合律}
\begin{Def}
集合$A$と集合$B$の二項関係$F\in A\times B$と集合$B$と集合$C$の二項関係$G\in B\times C$について, その{\bf 合成}$G\circ F\in A\times C$を
\[
...
\item $f:a\rightarrow b$が全射で, $a$が$U$の元であり, かつ$b$が$U$の部分集合ならば, $b$は$U$の元である
\end{enumerate}
\end{Def}
\begin{Def}
「宇宙」の元を{\bf 小集合}という.
...
\end{Prop}
\begin{Def}
$a$と$b$が小集合のとき関数$f:a\rightarrow b$を{\bf 小関数}という.
\end{Def}
\begin{Def}
「宇宙」の部分集合を{\bf 類}という.
\end{Def}
...
\begin{Def}
集合$G$と二項演算$\mu:G\times G\rightarrow G$が以下の条件を満たすとき$(G,\mu)$を{\bf 群}という
\begin{enumerate}
\item 結合法則
\item 単位元の存在
\item 逆元の存在
