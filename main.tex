...
\begin{Notation}
集合は各元を$\{\}$で囲むことで表される. 特に, とある条件を満たす$a$全体からなる集合は$\{a\mid a\text{についての条件}\}$を用いて表現する.
\end{Notation}
...
\end{Def}
\begin{example}
自然数全体$\mathbb{N}$, 整数全体$\mathbb{Z}$, 実数全体$\mathbb{R}$は順序集合である.
\end{example}
...
\end{Def}
\begin{Notation}
$f$が$A$から$B$への写像であることを$f:A\rightarrow B$で表す.
\end{Notation}
\begin{caution}
