...
\begin{Def}
圏$\mathscr{A}$において, 対象$A\in\mathrm{Obj}(\mathscr{A})$が以下の性質をもつとき, $A$を{\bf 圏$\mathscr{A}$の始対象 initial object}という.
\begin{itemize}
\item 圏$\mathscr{A}$の任意の対象$X\in\mathscr{A}$に対して, $A$から$X$への射が一意に存在する.
\end{itemize}
\end{Def}
\begin{Def}
圏$\mathscr{A}$において, 対象$A\in\mathrm{Obj}(\mathscr{A})$が以下の性質をもつとき, $A$を{\bf 圏$\mathscr{A}$の終対象 initial object}という.
\begin{itemize}
\item 圏$\mathscr{A}$の任意の対象$X\in\mathscr{A}$に対して, $X$から$A$への射が一意に存在する.
\end{itemize}
\end{Def}
\begin{Def}
{\bf　指数対象}
