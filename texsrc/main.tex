...
{\bf $\mathscr{A}$から$\mathscr{B}$への対象関数 function on objects}という.
\end{Def}
\begin{Def}
{\bf 射関数 function on morphisms}
\end{Def}
\begin{Def}
圏$\mathscr{A},\mathscr{B}$に対する対称関数と射関数の組を{\bf $\mathscr{A}$から$\mathscr{B}$への関手 functor}という.
\end{Def}
\begin{example}
順序を保存する写像
\end{example}
...
を{\bf 圏$A$から圏$B$への反変関手 contravariant functor}という.
\end{Def}
\subsection{定数関手}
\begin{Def}
圏$\mathscr{A}$の任意の対象$A$を圏$\mathscr{B}$のただ一つの対象$B_0$に写し,
$\mathscr{A}$の任意の射$f$を圏$\mathscr{B}$の恒等射に写す関手を
{\bf 定数関手 constant functor}\index{ていすうかんしゅ@定数関手}という
\end{Def}
